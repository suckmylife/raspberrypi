...
typedef struct {
    ...
    unsigned int biSizeImage; 		/* 이미지의 크기(압축 전 바이트 단위): 4바이트 */
...
typedef struct {
    unsigned char blue; 		/* 파란색을 위한 요소 */
    unsigned char green; 		/* 초록색을 위한 요소 */
    unsigned char red; 		/* 빨간색을 위한 요소 */
    unsigned char reserved; 		/* 투명도(알파값) 등을 위한 예약 공간 */
